...
import sys
sys.path.append(".")
from models.imagenet import resnet_50

class ResNet(nn.Module):
    def __init__(self, backbone='resnet50', backbone_path=None, weights="IMAGENET1K_V1", compress_rate=[0.0]*53, rank=1):
        super().__init__()
        if backbone == 'resnet18':
            ...
            backbone = resnet_50(compress_rate, rank)
            self.out_channels = [1024, 512, 512, 256, 256, 256]
        if backbone_path:
            backbone.load_state_dict(torch.load(backbone_path)['state_dict'])
        ...
        # conv4_block1.conv2.stride = (1, 1)
        conv4_block1.conv2.feature.vertical.stride = (1, 1)
        conv4_block1.conv2.feature.horizontal.stride = (1, 1)
        conv4_block1.downsample[0].stride = (1, 1)

        # sync last out_channels
        conv4_block6 = self.feature_extractor[-1][-1]
        self.out_channels[0] = conv4_block6.conv3.out_channels
...
class SSD300(nn.Module):
    def __init__(self, backbone=ResNet('resnet_50')):
        super().__init__()
