...
parser.add_argument("--input", type=str, default="data/", help="path to dataset")
parser.add_argument(
...
parser.add_argument(
    "--model-norton", type=str, default="norton-extreme.pt", help="norton model path"
)
...
def main():
    ...
    model_ori = resnet_50(compress_rate=[0.0] * 100, rank=0)
    model_hrank = resnet_50(compress_rate=compress_rate)
    model_chip = resnet_50(compress_rate=compress_rate)
    model_norton = resnet_50(compress_rate=compress_rate, rank=args.rank)
    ...
    state_dict_norton = torch.load(args.model_norton)
    model_norton.load_state_dict(state_dict_norton["state_dict"])
