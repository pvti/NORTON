...
from decomposition.decomposition import (
    cpdblock_weights_to_factors,
    factors_to_cpdblock_weights,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="pabs",
        choices=("pabs", "csa", "vbd"),
        help="criterion for similarity measure",
    )
...
args.job_dir = os.path.join(
    args.job_dir, args.arch, str(args.rank), args.criterion, args.compress_rate
)
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, CPDBlock):
            ...
            if ori_out_channels != cur_out_channels:
                ...
                ori_head_factor, ori_body_factor, ori_tail_factor = (
                    cpdblock_weights_to_factors(
                        ori_pointwise_weight,
                        ori_vertical_weight,
                        ori_horizontal_weight,
                        args.rank,
                    )
                )
                ...
                if last_select_index is not None:
                    ...
                    updated_head_factor = torch.empty(
                        (
                            cur_pointwise_in_channels,
                            ori_head_factor.size(1),
                            ori_head_factor.size(2),
                        )
                    )
                ...
                pointwise_weight, vertical_weight, horizontal_weight = (
                    factors_to_cpdblock_weights(head_factor, body_factor, tail_factor)
                )
                state_dict[name_base + pointwise_weight_name] = pointwise_weight
                state_dict[name_base + vertical_weight_name] = vertical_weight
                state_dict[name_base + horizontal_weight_name] = horizontal_weight

                for index_i, i in enumerate(select_index):
                    state_dict[name_base + bias_name][index_i] = ori_state_dict[
                        bias_name
                    ][i]
            ...
            # out_channels is identical but in_channels changed
                ...
                state_dict[name_base + vertical_weight_name] = ori_state_dict[
                    vertical_weight_name
                ]
                state_dict[name_base + horizontal_weight_name] = ori_state_dict[
                    horizontal_weight_name
                ]
                for i in range(ori_num_filter):
                    for index_j, j in enumerate(last_select_index):
                        state_dict[name_base + pointwise_weight_name][i][index_j] = (
                            ori_state_dict[pointwise_weight_name][i][j]
                        )
                last_select_index = None

            # none changes
            else:
                state_dict[name_base + pointwise_weight_name] = ori_state_dict[
                    pointwise_weight_name
                ]
                state_dict[name_base + vertical_weight_name] = ori_state_dict[
                    vertical_weight_name
                ]
            ...
            if last_select_index is not None:
                for index_i, i in enumerate(last_select_index):
                    state_dict[name_base + name + ".weight"][:, index_i] = (
                        ori_state_dict[name + ".weight"][:, i]
                    )
                last_select_index = None

            # linear2
            else:
                state_dict[name_base + name + ".weight"] = ori_state_dict[
                    name + ".weight"
                ]
            state_dict[name_base + name + ".bias"] = ori_state_dict[name + ".bias"]
...
def prune_resnet(model, ori_state_dict, num_layers=56):
        ...
        for k in range(num):
            for l in range(2):
                ...
                # out_channels is identical but in_channels changed
                    ...
                    for i in range(ori_num_filter):
                        for index_j, j in enumerate(last_select_index):
                            state_dict[name_base + pointwise_weight_name][i][
                                index_j
                            ] = ori_state_dict[pointwise_weight_name][i][j]
                    last_select_index = None
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, CPDBlock):
            ...
            logger.info(f"treat {name} which is not pruned")
            state_dict[name_base + name + ".weight"] = ori_state_dict[name + ".weight"]
            state_dict[name_base + name + ".bias"] = ori_state_dict[name + ".bias"]
...
def prune_densenet(model, ori_state_dict):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, CPDBlock):
            ...
            if cov_id == 1 or cov_id == 14 or cov_id == 27:
                ...
                tmp_select_index = [
                    x + cov_id * 12 - (cov_id - 1) // 13 * 12 for x in select_index
                ]
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Linear):
            logger.info(f"treat {name} which is not pruned")
            for index_i, i in enumerate(last_select_index):
                state_dict[name_base + name + ".weight"][:, index_i] = ori_state_dict[
                    name + ".weight"
                ][:, i]
            state_dict[name_base + name + ".bias"] = ori_state_dict[name + ".bias"]
...
def main():
    ...
    wandb.init(
        name=name, project=f"NORTON_Prune_{args.name}_{args.arch}", config=vars(args)
    )
    ...
    ori_model = eval(args.arch)(compress_rate=[0.0] * 100, rank=args.rank).cuda()
